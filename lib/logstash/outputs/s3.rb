...
class LogStash::Outputs::S3 < LogStash::Outputs::Base
  ...
  def create_temporary_file(prefix)
    ...
    @file_rotation_lock[prefix].synchronize do
      ...
      if @prefixes.include? prefix
         ...
         if @encoding == "gzip"
           @tempfile[prefix] = Zlib::GzipWriter.open(filename)
         else
  ...
  def write_to_tempfile(event, prefix)
    ...
    begin
      if @encoding == "gzip"
        @logger.debug("S3: put event into tempfile ", :tempfile => File.basename(@tempfile[prefix].path))
      else
        @logger.debug("S3: put event into tempfile ", :tempfile => File.basename(@tempfile[prefix]))
      end

      @file_rotation_lock[prefix].synchronize do
        @tempfile[prefix].write(event)
      end
    rescue Errno::ENOSPC
